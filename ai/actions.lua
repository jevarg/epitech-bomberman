...
function run_out_danger(map_nb, x, y, block)
	...
	for i = 1, BOMB_RANGE + 1 do
		if (block[1] == 0) then
			...
			elseif (i == BOMB_RANGE and map_nb[y - i][x] == ".") then if (i < nb) then nb = i ; way = 1 ; break end
	...
	for i = 1, BOMB_RANGE + 1 do
		if (block[2] == 0) then
			if (x - i < 1 or authorized_put_bomb(map_nb[y][x - i]) == 1) then block[2] = 1
			elseif (i == BOMB_RANGE and map_nb[y][x - i] == ".") then if (i < nb) then nb = i ; way = 3 ; break end
	...
	for i = 1, BOMB_RANGE + 1 do
		if (block[3] == 0) then
			if (y + i > MAP_YMAX or authorized_put_bomb(map_nb[y + i][x]) == 1) then block[3] = 1
			elseif (i == BOMB_RANGE and map_nb[y + i][x] == ".") then if (i < nb) then nb = i ; way = 2 ; break end
	...
	for i = 1, BOMB_RANGE + 1 do
		if (block[4] == 0) then
			if (x + i > MAP_XMAX or authorized_put_bomb(map_nb[y][x + i]) == 1) then block[4] = 1
			elseif (i == BOMB_RANGE and map_nb[y][x + i] == ".") then if (i < nb) then nb = i ; way = 4 ; break end
